...
namespace Wpf_Rescuemission
{
    ...
    public class TileGrid
    {
        ...
        public void TileChanges(string[] tokens)
        {
            Tile tile; 
            if (GetTileFromTokens(tokens[1], tokens[2], out tile))
            {
                switch (tokens[0])
                {
                    case "RemoveFirefighter":
                        tile.HasFirefighter = false;
                        break;
                    case "RemoveHazmat":
                        tile.setHazmat(false);
                        break;
                    case "RemoveFire":
                        tile.IsFire = false;
                        tile.IsSmoke = true;
        ...
        public void HandleChangedPosition(string[] tokens)
        {
            ...
            if (GetTileFromTokens(tokens[2],tokens[3], out fromTile) && GetTileFromTokens(tokens[5], tokens[6], out toTile))
            {
                ...
                {
                    case "Hazmat":
                        fromTile.setHazmat(false);
                        toTile.setHazmat(true);                       
                        break;
                    case "Victim":
                        fromTile.setVictim(false);
                        toTile.setVictim(true);
                        break;
                    case "Firefighter":
